# Projet : Apprentissage Fédéré + MIA + Défenses Optimisées + ROC/AUC + Attaque calibrée
...
# === Attaque MIA améliorée ===
def mia_score(model, x, y):
...
# === Construction jeux MIA ===
def get_mia_data():
    member_subset, _ = random_split(clients[0], [64, len(clients[0]) - 64])
    nonmember_subset, _ = random_split(test_data, [64, len(test_data) - 64])
    member_x, member_y = next(iter(DataLoader(member_subset, batch_size=64)))
    nonmember_x, nonmember_y = next(iter(DataLoader(nonmember_subset, batch_size=64)))
    return member_x, member_y, nonmember_x, nonmember_y

results = []
member_x, member_y, nonmember_x, nonmember_y = get_mia_data()

for defense in DEFENSES:
    for eps in EPSILONS:
        model = federated_learning(defense, eps)
        acc = evaluate(model, test_loader)
        loss_mem = mia_score(model, member_x, member_y)
        loss_non = mia_score(model, nonmember_x, nonmember_y)
        threshold = (np.mean(loss_mem) + np.mean(loss_non)) / 2
        tpr = (loss_mem < threshold).mean()
        fpr = (loss_non < threshold).mean()
        results.append({"defense": defense, "epsilon": eps, "accuracy": acc, "tpr": tpr, "fpr": fpr, "model": model, "scores": (loss_mem, loss_non)})
...
for j, defense in enumerate(DEFENSES):
    model_result = next(r for r in results if r['defense'] == defense and r['epsilon'] == 0.3)
    loss_mem, loss_non = model_result['scores']
    scores = np.concatenate([-loss_mem, -loss_non])
    labels = np.concatenate([np.ones_like(loss_mem), np.zeros_like(loss_non)])
