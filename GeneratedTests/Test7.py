# Projet : Apprentissage Fédéré + Attaque MIA + Défenses Multiples + Visualisation + ROC/AUC
...
# Stockage des modèles pour ROC/AUC
stored_models = {}
...
for defense in DEFENSES:
    for eps in EPSILONS:
        ...
        stored_models[(defense, eps)] = model
...
# === ROC/AUC pour chaque défense (epsilon = 0.3) ===
...
for j, defense in enumerate(DEFENSES):
    model = stored_models.get((defense, 0.3), None)
...
plt.title("Courbe ROC des attaques MIA (epsilon = 0.3)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
