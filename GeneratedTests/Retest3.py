...
def main():
    ...
    print("\n[MIA-Supervised] Attaque supervisée en cours...")

    def get_softmax_outputs(data):
        ...
        with torch.no_grad():
            for x, y in loader:
                x = x.to(DEVICE)
                outputs = torch.softmax(global_model(x), dim=1).cpu().numpy()
                softmax_outputs.extend(outputs)
                labels.extend(y.numpy())
        return np.array(softmax_outputs)

    member_features = get_softmax_outputs(member_data)
    nonmember_features = get_softmax_outputs(nonmember_data)
    ...
    plt.xlabel("FPR")
    plt.ylabel("TPR")
    plt.title("ROC - Membership Inference Attack (Supervised)")
    plt.legend()
    plt.grid(True)
    plt.show()
