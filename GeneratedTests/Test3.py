...
EPOCHS = 1
BATCH_SIZE = 64
EPSILONS = [0.0, 0.1, 0.3, 0.5, 0.7, 1.0]  # Différents niveaux de bruit
...
class MLP(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(28*28, 128),
            nn.ReLU(),
            nn.Dropout(0.4),  # Défense 1: régularisation
            nn.Linear(128, 10)
        )
...
# === Entraînement Local avec option de bruit ===
def train(model, loader, epsilon):
    model.train()
...
plt.figure()
plt.plot(EPSILONS, accs, label="Accuracy")
plt.plot(EPSILONS, tprs, label="TPR (MIA)")
plt.plot(EPSILONS, fprs, label="FPR (MIA)")
plt.xlabel("Epsilon (bruit)")
plt.ylabel("Métrique")
plt.title("Impact du bruit sur la performance et la robustesse")
plt.legend()
plt.grid(True)
plt.show()
